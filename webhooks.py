...
async def recibir_webhook_ml(request: Request, background: BackgroundTasks):
    ...
    try:
        ...
        if order_id and not shipment_id:
            ...
            # ⬇️ Fallback si /orders/{id} devolvió 401/403 o similar
            if od is None:
            ...
            if od:
                ...
                # Si igual no conseguimos shipment_id, no guardamos para no generar un registro “vacío”
                return {"ok": True, "mode": "order_no_shipment"}

        # --- Notificación por SHIPMENT (queda igual) ---
